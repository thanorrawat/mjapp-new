...
((Auth::user()->role_id==2||Auth::user()->role_id==7)  && ($cartDetails->order_status==12|| $cartDetails->order_status==22) ) //Manager อนุมัติใบจอง
          ...
          <form action="{{ url('confirmform') }}" method="post">
            <div class="text-center">
              <hr>
              <button type="submit" class="btn btn-success"><i class="fa fa-check-square-o" aria-hidden="true"></i> @if($cartDetails->order_status==22) อนุมัติใบจอง  @elseif($cartDetails->order_status==12) อนุมัติ Order @elseif($cartDetails->order_status==21) รับใบจอง @else รับOrder @endif</button>
              <br>
              <br>
