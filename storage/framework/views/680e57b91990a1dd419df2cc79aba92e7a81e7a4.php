...
<?php if(
(Auth::user()->role_id==6 && ($cartDetails->order_status==11|| $cartDetails->order_status==21) ) //admin รับ ORDER /ใบจอง
||
((Auth::user()->role_id==2||Auth::user()->role_id==7)  && ($cartDetails->order_status==12|| $cartDetails->order_status==22) ) //Manager อนุมัติใบจอง
): ?>
          <form action="<?php echo e(url('confirmform')); ?>" method="post">
            <div class="text-center">
              <hr>
              <button type="submit" class="btn btn-success"><i class="fa fa-check-square-o" aria-hidden="true"></i> <?php if($cartDetails->order_status==22): ?> อนุมัติใบจอง  <?php elseif($cartDetails->order_status==12): ?> อนุมัติ Order <?php elseif($cartDetails->order_status==21): ?> รับใบจอง <?php else: ?> รับOrder <?php endif; ?></button>
              <br>
              <br>
